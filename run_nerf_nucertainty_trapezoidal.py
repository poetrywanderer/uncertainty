...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    if render_poses.ndim == 3:
        ...
        raw.append(extras['raw'].cpu().numpy())
        z_vals.append(extras['z_vals'].cpu().numpy())
        alpha_com.append(extras['alpha_com'].cpu().numpy())
        weights_com.append(extras['weights_com'].cpu().numpy())
        '''
    ...
    pts = np.stack(pts, 0)
    raw = np.stack(raw, 0)
    z_vals = np.stack(z_vals, 0)
    alpha_com = np.stack(alpha_com, 0)
    weights_com = np.stack(weights_com, 0)
    '''
...
def raw2outputs_coarse(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False, coarse=True):
    ...
    """
    noise = 0.
    ## assume alpha follow a normal distribution with std=1
    if raw_noise_std > 0.:
    ...
    ## use sigmoid as activation for coarse rgb output
    rgb = torch.sigmoid(raw[...,:3])  # [N_rays, N_samples, 3]
    ## use relu as activation for coarse alpha output
    ...
    rgbs_map = torch.sum(0.5*(part_1+part_2), -2)  # [N_rays, 3]

    depth_map = torch.sum(weights_for_sample * z_vals, -1)
    disp_map = 1./torch.max(1e-10 * torch.ones_like(depth_map) + 1e-10, depth_map / (torch.sum(weights_for_sample, -1) + 1e-10) + 1e-10)
    acc_map = torch.sum(weights_for_sample, -1)

    # if white_bkgd:
    #     rgbs_map = rgbs_map + (1.-acc_map[:,None,:]) 

    return rgbs_map, weights_for_sample

def raw2outputs_fine(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False, coarse=True):
    """Transforms model's predictions to semantically meaningful values.
    Args:
        raw: [num_rays, num_samples along ray, 8]. Prediction from model.
        z_vals: [num_rays, num_samples along ray]. Integration time.
    ...
    rgb_var = torch.log(1 + torch.exp(raw[...,3])) + 1e-06 # [N_rays, N_samples]
    rgb_var = rgb_var.unsqueeze(-1)
    ## alpha var could be very high so causing overflow on exp()
    alpha_var = torch.log(1 + torch.exp(raw[...,5])) + 1e-06 # [N_rays, N_samples]
    # alpha_var = torch.relu(raw[...,5]) + 1e-06 # [N_rays, N_samples]
    ...
    time0 = time.time()
    k3 = 64
    rgb_xi = torch.randn(list(rgb_var.shape)+[k3]) / 100. # [N_rays, N_samples, 3, k3]
    alpha_xi = torch.randn(list(alpha_var.shape)+[k3]) / 100. # [N_rays, N_samples, k3]    
    rgbs = rgb_mean.unsqueeze(-1).expand(list(rgb_mean.shape)+[k3]) + rgb_xi * rgb_var.unsqueeze(-1).expand(list(rgb_mean.shape)+[k3])
    alphas = alpha_mean.unsqueeze(-1).expand(list(alpha_mean.shape)+[k3]) + alpha_xi * alpha_var.unsqueeze(-1).expand(list(alpha_var.shape)+[k3])

    ## use sigmoid as activation for sampled rgb output
    rgb = torch.sigmoid(rgbs)  # [N_rays, N_samples, 3, k3]

    dists = z_vals[...,1:] - z_vals[...,:-1]
    dists = torch.cat([dists, torch.Tensor([1e10]).expand(dists[...,:1].shape)], -1)  # [N_rays, N_samples]
    dists = dists * torch.norm(rays_d[...,None,:], dim=-1)

    alpha = torch.relu(alphas) # [N_rays, N_samples, k3]

    # print('alpha_mean max:', torch.max(alpha_mean))
    # print('alpha_mean min:', torch.min(alpha_mean))
    # print('alpha_xi max:', torch.max(alpha_xi))
    # print('alpha_xi min:', torch.min(alpha_xi))
    # print('alpha_var max:', torch.max(alpha_var))
    # print('alpha_var min:', torch.min(alpha_var))
    # print('alpha max:', torch.max(alpha))
    # print('alpha min:', torch.min(alpha))

    ## trapezoidal rule
    a1 = torch.exp(-0.5*alpha * dists.unsqueeze(-1).expand(alpha.shape)) # [N_rays, N_samples, k3]
    a2 = torch.exp(-0.5*torch.cat([alpha,torch.ones((alpha.shape[0], 1, k3))],-2)[:,1:,:] * dists.unsqueeze(-1).expand(alpha.shape)) # [N_rays, N_samples, k3]
    weights_1 = torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1, k3)), a1], -2), -1)[:, :-1, :] # [N_rays, N_samples, k3]
    weights_2 = torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1, k3)), a2], -2), -1)[:, :-1, :] # [N_rays, N_samples, k3]
    weights_com = weights_1 * weights_2 * alpha # [N_rays, N_samples, k3]
    weights_for_sample = weights_com * torch.cat([torch.ones((alpha.shape[0], 1)), dists], -1)[...,None][:,:-1,:] # [N_rays, N_samples, k3]
    weights_com = weights_com[...,None,:] * rgb # [N_rays, N_samples, 3, k3]

    # print(weights_1)
    # print('weights_1 max',torch.max(weights_1))
    # print('weights_1 min',torch.min(weights_1))

    assert a1.isinf().any() == False
    assert a2.isinf().any() == False
    assert weights_1.isinf().any() == False
    assert weights_2.isinf().any() == False
    
    part_1 = weights_com * torch.cat([torch.ones((alpha.shape[0], 1)), dists], -1)[...,None,None][:,:-1,:,:] # [N_rays, N_samples, 3, k3]

    part_2 = torch.cat([torch.zeros((alpha.shape[0], 1, 3, k3)), weights_com], -3)[:,:-1,:,:] * torch.cat([torch.ones((alpha.shape[0], 1)), dists], -1)[...,None,None][:,:-1,:,:] # [N_rays, N_samples, 3, k3]

    rgbs_map = torch.sum(0.5*(part_1+part_2), -3)  # [N_rays, 3, k3]

    depth_map = torch.sum(weights_for_sample[...,0] * z_vals, -1)
    disp_map = 1./torch.max(1e-10 * torch.ones_like(depth_map) + 1e-10, depth_map / (torch.sum(weights_for_sample[...,0], -1) + 1e-10) + 1e-10)
    acc_map = torch.sum(weights_for_sample, -2)

    if white_bkgd:
        rgbs_map = rgbs_map + (1.-acc_map[:,None,:])

    return rgbs_map, disp_map, acc_map, weights_for_sample, depth_map, alpha
...
def render_rays(ray_batch,
    ...
    # rgbs_map, weights = raw2outputs_coarse(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)
    time0 = time.time()
    rgbs_map, weights = raw2outputs_coarse(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)
    # print('coarse time:', time.time()- time0)
    ## fine process
    rgbs_map_0 = rgbs_map
    ...
    time0 = time.time()
    rgbs_map, disp_map, acc_map, weights_com, depth_map, alphas = raw2outputs_fine(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)
    # print('fine time:', time.time()- time0)
    ret = {'rgb_map' : rgbs_map, 'disp_map' : disp_map, 'acc_map' : acc_map}
    if retraw:
        ret['raw'] = raw
        ret['pts'] = pts
        ret['z_vals'] = z_vals
        ret['weights_com'] = weights_com
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if 'rgb0' in extras:
            ...
            loss_0 = torch.mean((extras['rgb0']-target_s)**2)
            loss += loss_0
            psnr_train0 = mse2psnr(loss_0)
            scalars_to_log['train/mse0'] = loss_0.item()
            scalars_to_log['train/psnr0'] = psnr_train0.item()
        
        ## compute entropy using monto-carlo estimator
        # lamda_r = 10
        # loss_regularization = lamda_r*torch.mean((extras['alphas'])**2)
